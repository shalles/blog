...
<a id="mobile"></a>
如在我的手机上, 代理主机名就是fiddler服务器的`计算机`名或`IP地址`，注意fiddler默认监控的端口是`8888`, [这个可以配置](/tools/fiddler/2015/02/08/tools-fiddler-profile2/#Connections)
...
c. <a id="reqResBreakPoint"></a>第三个是会话request发送前的断点和response返回后且到浏览器前的断点，分别对应点一次，点两次该按钮（默认是空白即不设断点），[request断点和response断点详情](/tools/fiddler/2015/05/18/tools-fiddler-function-list/#beforeRequest)中详解
...
快速执行命令行,关于命令的[详情请看](/tools/fiddler/translate/0000/04/11/Translate-fiddler-quick-exec)
